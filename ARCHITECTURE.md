...
|--------|-----------|------|----------------|
| VTA | brain/dopamine/vta-state.json | palantir-vta-drive decay | ✅ drive level |
| Amygdala (Amy) | brain/amy/emotional-decay-state.json | palantir-emotion-decay | ✅ valence |
| Insula (Somatic) | brain/somatic/markers.json | palantir-somatic-monitor | ✅ gut/energy/tension |
| Hippocampus | brain/hippocampus/dream-state.json | palantir-memory-replay | ⚠️ partial |
...
| Attention | brain/attention/attention-state.json | palantir-attention-wander | ✅ mode (focused/distracted) |
| Fatigue | brain/fatigue/fatigue-state.json | palantir-fatigue-monitor | ✅ recovery_needed, level |
| Values | brain/values/values-state.json | palantir-value-updater | ⚠️ decoupled from gate |
| Predictive | brain/predictive/predictor-state.json | palantir-surprise-analyzer | ⚠️ predictions not read by gate |
| ToM | brain/tom/observation-log.json | palantir-tom-observer | ⚠️ self-model not read by gate |
| Novelty | brain/dopamine/novelty-state.json | palantir-novelty-detector | ✅ novelty_seeking |
...
|------|-------------|--------|-------|------------|
| palantir-autonomous-cycle | b0ffd2c0 | ✅ ok | ✅ YES | — |
| palantir-somatic-monitor | 9a88bad5 | ❌ error | ✅ YES | — |
| palantir-fatigue-monitor | c54f176c | ✅ ok | YES | — |
| palantir-novelty-detector | 6cffdab6 | ✅ ok | YES | — |
| palantir-surprise-analyzer | 955ee39d | ✅ ok | YES | — |
| palantir-predictive-monitor | bfecef80 | ✅ ok | YES | — |
| palantir-tom-observer | 8aac6872 | ✅ ok | YES | — |
| palantir-tom-self-bridge | 5cfd5df3 | ✅ ok | YES | — |
| palantir-attention-wander | 73624347 | ✅ ok | YES | — |
| palantir-value-updater | 7c10cdcc | ❌ error | YES | — |
| palantir-emotional-tone | e263971a | ❌ error | YES | — |
| palantir-dream-imagine | 47db4e5b | ✅ ok | YES | — |
| palantir-memory-replay | ad63bfc1 | ✅ ok | YES | — |
| palantir-moltx-engage | 48955fe5 | ✅ ok | — | YES |
| palantir-moltbook | c1eddfcf | ❌ error | — | YES |
| palantir-memory-janitor | 24d49ccc | ✅ ok | — | — |
| palantir-autonomous-tasks | b0ffd2c0 | ✅ ok | YES | — |
| isildur-tts-healthcheck | 2ad2e771 | ⚠️ intermittent | — | — |

### ❌ ERROR CRONS (6):

1. **palantir-somatic-monitor** — Reports error but runs fine manually. Likely timeout exit code or stderr noise. Not actually broken.

2. **palantir-value-updater** — Script exists but has bash multiline-string bug (line 3 interpreted as command). Produces output but exits non-zero. **Fix: remove descriptive comment from line 2 of script.**

3. **palantir-emotional-tone-calibration** — Same issue as value-updater. Script exists at `brain/amy/scripts/emotion-decay.sh` but cron may reference wrong path.

4. **palantir-moltbook-engagement** — 401 Unauthorized. API key not in cron environment. NOT the same as the 500 errors from earlier (those were server-side). **Requires: API key in cron env or browser-based posting.**
...
3. **Essential flag missing on most social crons** — `palantir-moltx-engage` has essential=true, but `palantir-moltbook-engage` doesn't (and it's erroring anyway).
